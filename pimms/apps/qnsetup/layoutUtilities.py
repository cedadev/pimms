...
def getsims(centre):
    ...
    for s in tablesims:
        ...
        s.copysimurl=reverse('pimms.apps.qn.views.simulationCopyInd', 
                         args=(centre.id,s.id))
        #get individual sim delete url (for non-published sims)
        if len(CIMObject.objects.filter(uri=s.uri)):
            s.delurl = None
        else:     
            s.delurl=reverse('pimms.apps.qn.views.simulationDel', 
                         args=(centre.id,s.id))     
...
class tabs(list):
    ...
    def __init__(self,request,centre_id,page,object_id=0):
        ...
        self.tablist=[
            ('Summary', 'pimms.apps.qn.views.centre', (centre_id,), 'left'),
            ('Experiments', 'pimms.apps.qn.views.simulationList', 
                 (centre_id,), 'left'),
            ('Model', 'pimms.apps.qn.views.componentEdit', 
            ...
            ('Files', 'pimms.apps.qn.views.list', 
                 (centre_id, 'file',), 'left'),
            ('References', 'pimms.apps.qn.views.list', 
                 (centre_id, 'reference',), 'left'),
            ('Parties', 'pimms.apps.qn.views.list', 
                 (centre_id, 'parties',), 'left'),
            ('Help', 'pimms.apps.qn.views.help', 
                 (centre_id,), 'right'),
            ('About', 'pimms.apps.qn.views.about', 
                 (centre_id,), 'right'),
            ('Log Out', 'right'),
            ]
